...
class SequenceToSequence(object):
    ...
    def build_model(self):
        """
            构建整个模型
        分别构建
        编码器（encoder）  build_encoder ->  encoder_cell  -> build_single_cell
        解码器（decoder）
    ...
    def build_single_cell(self, n_hidden, use_residual):
        ...
        if use_residual:
            cell = ResidualWrapper(cell)  #残差网络
    ...
    def save(self, sess, save_path='model.ckpt'):
        """保存模型
        tensorflow 里有两种保存方式：
        ckpt: 训练模型后的保存，这里会保存所有的训练参数，文件相对比较大，可以用来数据的恢复和加载
        pd: 用于模型最后的上线部署，这里的线上部署是指TensorFlow Serving进行模型的发布，一版发布成qrpc形式的接口
        """
    ...
    def check_feeds(self, encoder_inputs, encoder_inputs_length,
        ...
        if not decode:
            ...
            if target_batch_size != decoder_inputs_length.shape[0]:
                raise ValueError(
                    "encoder_inputs和decoder_inputs_length的第一维度必须一致 "
                    "这一维度是batch_size, %d != %d" % (
